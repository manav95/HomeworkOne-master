...
void print_files(const std::vector<unsigned int>& solutions, unsigned int n) {
      std::ofstream myfile;
      myfile.open ("/home/student/Downloads/mlm/sp16-cse2000/example.txt");
      size_t num_sols = solutions.size() / n;
      for (size_t i = 0; i < num_sols; ++i) {
          for (unsigned int j = 0; j < n; ++j) {
              if (j != 0)
                 myfile << " ";
              myfile << solutions[i*n + j];
          }
          myfile << std::endl;
      }
      myfile.close(); 
