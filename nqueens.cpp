...
void nqueens_by_level(std::vector<unsigned int> pos, unsigned int start_level,
    ...
    // TODO!
    // (i, j) (k, l)
    if (start_level == max_level) { // edge case
        success_func(pos);
    }
    unsigned int l = start_level; // column to check
    for (unsigned int k = 0; k < pos.size(); k++) { // goes through possible rows
        unsigned int j = 0;
        unsigned int i = pos[j];
        while(j < start_level && !(i==k || j==l || (i-k)==(j-l) || -(i-k)==(j-l))) {
            j++;
            i = pos[j];
        }
        if (j == start_level) {
            pos[l] = k;
            if (start_level == max_level - 1) { // if reached max level
                success_func(pos);
            } else {
